...
\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de potências e escalas.}
]{codigos/cap01/saidas/potencias_e_escalas.txt}
...
\subsection{Notação científica em Python}

Python aceita a letra \texttt{e} para representar potências de 10. Assim, \texttt{4.8e-6} corresponde a $4{,}8\times10^{-6}$. A especificação de formato \texttt{.2e} apresenta um número em notação científica com duas casas decimais.

\lstinputlisting[
  caption={Representação e operações com notação científica.},
  label={lst:notacao-cientifica}
]{codigos/cap01/notacao_cientifica.py}

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de notação científica.}
]{codigos/cap01/saidas/notacao_cientifica.txt}
...
Se multiplicarmos $x$ por um fator $f$, o mesmo acontecerá com $y$:
...
Portanto, triplicar a velocidade multiplica a energia cinética por nove. A dependência quadrática faz a energia crescer mais rapidamente que a velocidade.
...
\begin{enumerate}
  ...
  \item Com velocidade constante, o que ocorre com $E_c$ se a massa quintuplica?
  \item Com massa constante, o que ocorre com $v$ para $E_c$ quadruplicar?
  \item Considere $P\propto A^2/(BC)$. Determine a mudança em $P$ quando $A$ dobra, $B$ triplica e $C$ é reduzido à metade.
  \item Explique por que uma proporcionalidade correta pode deixar de representar um sistema físico real.
\end{enumerate}
...
O argumento de um logaritmo deve ser adimensional. Por isso, expressões físicas logarítmicas usam razões entre grandezas de mesma dimensão, como
...
\subsection{Logaritmos em Python}

O módulo \texttt{math} oferece \texttt{log10} para o logaritmo decimal e \texttt{log} para o logaritmo natural. No mesmo programa, usamos escalas logarítmicas nos dois eixos para visualizar a lei $y=3x^2$.

\lstinputlisting[
  caption={Logaritmos, decaimento exponencial e gráfico log-log.},
  label={lst:logaritmos}
]{codigos/cap01/logaritmos.py}

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de logaritmos.}
]{codigos/cap01/saidas/logaritmos.txt}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{cap01/lei-potencia-loglog.pdf}
  \caption{A lei de potência $y=3x^2$ em escalas logarítmicas. A inclinação constante corresponde ao expoente 2.}
  \label{fig:lei-potencia-loglog}
\end{figure}
...
A \cref{fig:componentes-forca} apresenta primeiro o vetor original e, depois, suas projeções sobre os eixos cartesianos. As linhas tracejadas completam o retângulo de decomposição e ajudam a visualizar a soma vetorial $\vetor{F}=\vetor{F}_x+\vetor{F}_y$.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{cap01/componentes-forca.pdf}
  \caption{Uma força de \SI{80}{\newton} a $30^\circ$ da horizontal e sua decomposição. No segundo painel, $F_x\approx\SI{69.3}{\newton}$ e $F_y=\SI{40}{\newton}$.}
  \label{fig:componentes-forca}
\end{figure}

O programa usado para produzir a figura aplica diretamente as relações calculadas no exemplo. O método \texttt{quiver}, introduzido no \cref{sec:primeiros-graficos}, recebe a origem da seta e suas componentes horizontal e vertical.

\lstinputlisting[
  caption={Representação da força e de suas componentes cartesianas.},
  label={cod:componentes-forca}
]{codigos/cap01/componentes_forca.py}

A saída textual esperada é

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de componentes de uma força.}
]{codigos/cap01/saidas/componentes_forca.txt}

Além de confirmar os valores, o desenho permite verificar geometricamente que $F^2=F_x^2+F_y^2$.

\subsection{Seno e cosseno em Python}

As funções \texttt{sin} e \texttt{cos} do módulo \texttt{math} recebem ângulos em radianos. Por isso, o programa converte graus com \texttt{math.radians} antes do cálculo. Ele também produz o círculo trigonométrico da \cref{fig:circulo-trigonometrico} e os gráficos periódicos da \cref{fig:seno-cosseno}.

\lstinputlisting[
  caption={Valores de seno e cosseno e geração das figuras trigonométricas.},
  label={lst:trigonometria}
]{codigos/cap01/trigonometria.py}

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de trigonometria.}
]{codigos/cap01/saidas/trigonometria.txt}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.62\textwidth]{cap01/circulo-trigonometrico.pdf}
  \caption{No círculo unitário, as coordenadas do ponto são $\cos\theta$ e $\sin\theta$. As linhas tracejadas mostram suas projeções nos eixos.}
  \label{fig:circulo-trigonometrico}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{cap01/seno-cosseno.pdf}
  \caption{Seno e cosseno ao longo de uma volta completa. O estilo tracejado diferencia o cosseno mesmo em impressão sem cores.}
  \label{fig:seno-cosseno}
\end{figure}
...
\subsection{Representação de funções em Python}

Na \cref{sec:primeiros-graficos}, construímos gráficos a partir de listas e arrays. Para representar uma função matemática, seguimos o mesmo processo: escolhemos um domínio amostrado, calculamos a imagem de cada ponto e entregamos os dois arrays ao método \texttt{plot}.

O programa a seguir define três funções com \texttt{def}. Como as operações usadas por elas são compatíveis com arrays NumPy, uma única chamada, como \texttt{funcao\_quadratica(x)}, calcula todos os valores necessários. Essa aplicação de uma operação elemento a elemento é chamada de \textbf{vetorização}.

\lstinputlisting[
  caption={Representação de funções linear, quadrática e inversa.},
  label={cod:plotando-funcoes}
]{codigos/cap01/plotando_funcoes.py}

A saída textual esperada é

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de representação de funções.}
]{codigos/cap01/saidas/plotando_funcoes.txt}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{cap01/funcoes-basicas.pdf}
  \caption{Três funções calculadas no domínio amostrado $0{,}2\leq x\leq4{,}0$. Cada painel possui seus próprios eixos e representa uma relação matemática diferente.}
  \label{fig:funcoes-basicas}
\end{figure}

O domínio começa em \num{0.2}, e não em zero, porque a função $1/x$ não está definida em $x=0$. Essa decisão ilustra uma regra importante: o programa não escolhe automaticamente um domínio matematicamente válido ou fisicamente significativo. Essa responsabilidade pertence a quem constrói o modelo.

No laço \texttt{for}, a função \texttt{zip} agrupa, em cada repetição, um sistema de eixos, uma função e seu rótulo. Assim, evitamos repetir o mesmo conjunto de comandos para cada painel. A técnica é conveniente aqui porque os três gráficos compartilham a mesma estrutura; ela não deve esconder diferenças importantes entre grandezas, unidades ou domínios.
...
\subsection{Derivada analítica e derivada numérica}

Para $f(x)=x^2$, a derivada analítica é $f'(x)=2x$. O programa também estima a derivada numericamente por uma diferença central com intervalo pequeno. A concordância das duas abordagens em $x=2$ valida o cálculo, enquanto a \cref{fig:derivada-tangente} mostra a interpretação geométrica da inclinação.

\lstinputlisting[
  caption={Comparação entre derivada analítica e diferença central.},
  label={lst:derivadas}
]{codigos/cap01/derivadas.py}

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de derivadas.}
]{codigos/cap01/saidas/derivadas.txt}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{cap01/derivada-tangente.pdf}
  \caption{A derivada de $f(x)=x^2$ em $x=2$ é a inclinação da reta tangente no ponto $(2,4)$.}
  \label{fig:derivada-tangente}
\end{figure}

\subsection{Regras de derivação mais utilizadas}

Sejam $u(x)$ e $v(x)$ funções diferenciáveis e $c$ uma constante. As regras seguintes serão usadas repetidamente ao longo do livro.

\subsubsection*{Constante, multiplicação por constante e soma}

\begin{align}
  \frac{\dd}{\dd x}c &= 0,\\
  \frac{\dd}{\dd x}\bigl[c\,u(x)\bigr]
    &=c\,u'(x),\\
  \frac{\dd}{\dd x}\bigl[u(x)\pm v(x)\bigr]
    &=u'(x)\pm v'(x).
\end{align}

Essas regras permitem derivar uma expressão termo a termo. Por exemplo,

\begin{equation}
  \frac{\dd}{\dd t}\left(x_0+v_0t+\frac{1}{2}at^2\right)
  =v_0+at.
\end{equation}

\subsubsection*{Produto e quociente}

\begin{align}
  \frac{\dd}{\dd x}\bigl[u(x)v(x)\bigr]
    &=u'(x)v(x)+u(x)v'(x),\\
  \frac{\dd}{\dd x}\left[\frac{u(x)}{v(x)}\right]
    &=\frac{u'(x)v(x)-u(x)v'(x)}{[v(x)]^2},
    \qquad v(x)\neq0.
\end{align}

Não se deve derivar um produto multiplicando as derivadas, nem um quociente dividindo-as. As expressões completas acima são necessárias.

\subsubsection*{Regra da cadeia}

Quando uma função está dentro de outra, usamos

\begin{equation}
  \frac{\dd}{\dd x}f\bigl(u(x)\bigr)
  =f'\bigl(u(x)\bigr)u'(x).
\end{equation}

Derivamos a função externa e multiplicamos pela derivada da função interna. Dois exemplos importantes em Física são

\begin{align}
  \frac{\dd}{\dd t}\cos(\omega t+\phi)
    &=-\omega\sin(\omega t+\phi),\\
  \frac{\dd}{\dd t}\e^{-t/\tau}
    &=-\frac{1}{\tau}\e^{-t/\tau}.
\end{align}

\subsubsection*{Derivadas elementares}
...
As fórmulas trigonométricas pressupõem que o argumento esteja em radianos. Regras específicas para outras funções e derivadas implícitas serão introduzidas quando forem necessárias.
...
\subsection{Integral analítica e integração numérica}

Para $v(t)=2t$, a integral entre 0 e \SI{4}{\second} vale \SI{16}{\meter}. O programa compara esse resultado com a regra dos trapézios implementada por \texttt{numpy.trapezoid}. A região sombreada na \cref{fig:integral-area} representa o deslocamento acumulado.

\lstinputlisting[
  caption={Comparação entre integral analítica e integração numérica.},
  label={lst:integrais}
]{codigos/cap01/integrais.py}

\lstinputlisting[
  style=saidaesperada,
  caption={Saída esperada do programa de integrais.}
]{codigos/cap01/saidas/integrais.txt}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{cap01/integral-area.pdf}
  \caption{O deslocamento é a área sob o gráfico de velocidade por tempo. Para $v(t)=2t$, a área triangular vale \SI{16}{\meter}.}
  \label{fig:integral-area}
\end{figure}

\subsection{Regras de integração mais utilizadas}

Uma antiderivada de $f(x)$ é uma função $F(x)$ cuja derivada é $f(x)$. Escrevemos

\begin{equation}
  \int f(x)\,\dd x=F(x)+C,
\end{equation}

onde $C$ é a constante de integração. Ela aparece porque funções que diferem apenas por uma constante possuem a mesma derivada.
...
\begin{align}
  \int c\,f(x)\,\dd x
    &=c\int f(x)\,\dd x,\\
  \int\bigl[f(x)\pm g(x)\bigr],\dd x
    &=\int f(x)\,\dd x
      \pm\int g(x)\,\dd x.
\end{align}

Assim como na derivação, uma soma pode ser tratada termo a termo. Entretanto, não existe uma regra geral que permita integrar um produto apenas multiplicando integrais.

\subsubsection*{Integrais elementares}
...
Os fatores $1/a$ aparecem por causa da regra da cadeia. Podemos verificar cada resultado derivando o lado direito.

\subsubsection*{Integral definida e teorema fundamental}
...
Em uma integral definida, a constante $C$ não aparece no resultado porque seria cancelada na diferença entre os limites. O valor representa uma acumulação líquida: contribuições positivas e negativas são combinadas algebricamente.

Integração por substituição, integração por partes e outras técnicas serão apresentadas apenas quando os modelos físicos exigirem seu uso.
...
Essa condição identifica pontos candidatos, mas não os classifica. Se
...
Programas de computador automatizam operações e ampliam nossa capacidade de exploração. Entretanto, não escolhem sozinhos o sistema, as hipóteses nem o significado físico do resultado. Essa responsabilidade permanece com quem modela.
